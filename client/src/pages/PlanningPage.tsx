...
export default function PlanningPage() {
  ...
  // Handle landmark click → open details panel
  const handleLandmarkClick = useCallback((lm: any) => {
    setSelectedLandmark(lm);
  }, []);

  // Add landmark as waypoint (from panel)
  const addLandmarkAsWaypoint = useCallback(async () => {
    ...
    try {
      await api.post(`/waypoints/trip/${id}`, {
        ...
        description: selectedLandmark.description?.substring(0, 500) || 'Famous US landmark',
      });
      toast.success(`Added "${selectedLandmark.name}"`);
      if (mapRef.current) {
        mapRef.current.flyTo({ center: [selectedLandmark.longitude, selectedLandmark.latitude], zoom: 10, duration: 1500 });
      }
